...
async function createJwt(appId: string, privateKey: string): Promise<string> {
    ...
    return await sign(payload, privateKey, { algorithm: 'RS256' });
}

async function getInstallationAccessToken(appId: string, installationId: string, jwt: string): Promise<string> {
    ...
    if (!data.token) {
        throw new Error(`Failed to get installation access token: ${JSON.stringify(data)}`);
    }
    return data.token;
}

async function getRepositoryId(repository: string, token: string): Promise<string> {
...
export async function createGithubDiscussion(
    ...
    if (!data.data || !data.data.createDiscussion) {
        throw new Error(`Failed to create GitHub discussion: ${JSON.stringify(data)}`);
    }
